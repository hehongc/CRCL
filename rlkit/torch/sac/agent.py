...
class PEARLAgent(nn.Module):
    ...
    def __init__(self,
        ...
        self.W = nn.Parameter(torch.rand(self.latent_dim, self.latent_dim)).cuda()
    ...
    def only_infer_context_embeddings(self, context, task_indices=None):
        ''' compute q(z|c) as a function of input context and sample new z from it'''
        params = self.context_encoder(context)
        params = params.view(context.size(0), -1, self.context_encoder.output_size)
        ...
        z_embeddings = z_means

        return z_embeddings

    def only_infer_context_embeddings_TACO(self, context, task_indices=None):
        ''' compute q(z|c) as a function of input context and sample new z from it'''
        params = self.context_encoder(context)
        params = params.view(context.size(0), -1, self.context_encoder.output_size)
