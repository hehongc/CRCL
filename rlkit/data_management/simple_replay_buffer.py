...
class SimpleReplayBuffer(ReplayBuffer):
    def __init__(
        ...
        self._observations = np.zeros((max_replay_buffer_size, observation_dim), dtype="float32")
        # It's a bit memory inefficient to save the observations twice,
        # but it makes the code *much* easier since you no longer have to
        # worry about termination conditions.
        self._next_obs = np.zeros((max_replay_buffer_size, observation_dim), dtype="float32")
        self._actions = np.zeros((max_replay_buffer_size, action_dim), dtype="float32")
        # Make everything a 2D np array to make it easier for other code to
        # reason about the shape of the data
        self._rewards = np.zeros((max_replay_buffer_size, 1), dtype="float32")
        self._sparse_rewards = np.zeros((max_replay_buffer_size, 1), dtype="float32")
    ...
    def random_sequence(self, batch_size):
        ...
        start = np.random.randint(0, self._size, 1)
        start = start[0]
        if start + batch_size > self._size:
            indices = list(range(start, self._size)) + list(range(0, batch_size + start - self._size ))
        else:
